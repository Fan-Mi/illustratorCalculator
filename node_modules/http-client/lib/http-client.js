 ...
 * @name HTTPClient
 */
var HTTPClient = module.exports = function (agent, timeout) {
 ...
 * if String: Alias for GET request, equivalent { url : '_url_' }
...
HTTPClient.prototype.open = function (connection, callback) {
    ...
    return new HTTPRequest(options)
        .once('request', function (request) {
            ...
            request.setTimeout(self.timeout || 0, function(){
                request.abort();
                self.timeout = -1;
            });
	    ...
	    var dateFail   = new Date();
            var errmsgpost = (-1 === self.timeout) ? ( dateFail + ' ontos failed to answer within timeout') : '';
            callback(err.toString() + errmsgpost);
        })
        .open();
};
