...
exports.testGetRequest = function (test) {
    ...
    client.open('http://www.yandex.ru/yandsearch?text=test', function (err, page) {
        test.ifError(err, 'HttpClient.open returns error');
        test.equal(typeof page, 'string', 'document is not a string');
        test.ok(page.length > 0, 'page length is null');
        test.ok(!!~page.search(/html/i), 'page is not html');
        test.done();
    });
};
...
exports.testPostRequest = function (test) {
    ...
    client.open({ url : 'http://translate.google.com/', data : data }, function (err, page) {
        ...
        test.ok(!!~page.search('The quick brown fox jumps over the lazy dog'));
        test.done();
    });
};
